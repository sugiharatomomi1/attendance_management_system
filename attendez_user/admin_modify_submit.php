...
if ($sql_check_student->fetch()) { // 学籍番号がある場合
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sql_check_existing_message = $pdo->prepare('SELECT * FROM student_message 
                                                    WHERE student_number = ? AND date = ?');
        $sql_check_existing_message->execute([$student_number, $currentDate]);

        if ($sql_check_existing_message->fetch()) {
            // 既存データがある場合は更新
            $sql_update_message = $pdo->prepare('UPDATE student_message SET text = ?
                                                WHERE student_number = ? AND date = ?');
            $sql_update_message->execute([$message, $student_number, $currentDate]);
        } else {
            // 既存データがない場合は新規追加
            $sql_insert_message = $pdo->prepare('INSERT INTO student_message (id, student_number, date, text) 
                                                VALUES (null, ?, ?, ?)');
            $sql_insert_message->execute([$student_number, $currentDate, $message]);
